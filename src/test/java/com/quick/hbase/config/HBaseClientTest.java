...
public class HBaseClientTest {
    ...
    public void add() throws IOException {
        try {
            ArrayList<String> work = ReadFiles.work();
            for (String s : work) {
                String[] split = s.split(",");
                hBaseClient.insertOrUpdate(TABLE, split[0], "INFO", "DATA_TIME", split[1]);
                hBaseClient.insertOrUpdate(TABLE, split[0], "INFO", "POINT_NAME", split[2]);
                hBaseClient.insertOrUpdate(TABLE, split[0], "INFO", "POINT_VALUE", split[3]);
                hBaseClient.insertOrUpdate(TABLE, split[0], "INFO", "POINT_QUALITY", split[4]);
                hBaseClient.insertOrUpdate(TABLE, split[0], "INFO", "PULL_TIME", split[5]);
            }
        ...
        hBaseClient.insertOrUpdate(TABLE, "1", "INFO", "DATA_TIME", "1km/h");
        hBaseClient.insertOrUpdate(TABLE, "1", "INFO", "POINT_NAME", "better");
        hBaseClient.insertOrUpdate(TABLE, "1", "INFO", "POINT_VALUE", "create table");
        hBaseClient.insertOrUpdate(TABLE, "1", "INFO", "POINT_QUALITY", "2019年07月20日17:52:53");
        hBaseClient.insertOrUpdate(TABLE, "1", "INFO", "PULL_TIME", "admin");
